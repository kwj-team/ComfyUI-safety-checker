...
class ClipSafetyChecker(PreTrainedModel):
    ...
    @staticmethod
    def remove_nsfw_images(images, nsfw_concepts):
        # Remove nsfw images from the batch
        new_images = []
    ...
    def forward(self, clip_input, images, sensitivity):
        try:
            with torch.no_grad():
                ...
                special_cos_dist = ClipSafetyChecker.compute_cosine_similarity(
                    image_embeds, self.special_care_embeds)
                cos_dist = ClipSafetyChecker.compute_cosine_similarity(
                    image_embeds, self.concept_embeds)
                ...
                special_scores = special_cos_dist - \
                    self.special_care_embeds_weights.unsqueeze(
                        0) + adjusted_sensitivity
                concept_scores = cos_dist - \
                    self.concept_embeds_weights.unsqueeze(
                        0) + adjusted_sensitivity

                # Process scores for the entire batch
                results = ClipSafetyChecker.process_image_scores_batch(
                    special_scores, concept_scores)

                # Check for NSFW content and replace corresponding images
                nsfw_concepts = [len(res["bad_concepts"])
                                 > 0 for res in results]
                images = ClipSafetyChecker.remove_nsfw_images(
                    images, nsfw_concepts)

                # Log a warning if any NSFW content is detected
                if any(nsfw_concepts):
                    logging.warning(
                        "Safety Checker: NSFW content detected. Removed.")
...
class Safety_Checker:
    def __init__(self):
        try:
            ...
            self.safety_checker = ClipSafetyChecker.from_pretrained(
                safety_checker_model)
        except Exception as e:
    ...
    def nsfw_checker(self, images, sensitivity):
        try:
            ...
            checked_image, nsfw = self.safety_checker(
                images=images, clip_input=safety_checker_input.pixel_values, sensitivity=sensitivity)
    ...
    def censor_batch(self, x):
        try:
            ...
            x_checked_image, has_nsfw_concept = self.nsfw_checker(
